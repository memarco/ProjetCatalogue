...
class Search extends CI_Controller
{
	...
	{
		parent::__construct();
		$this->load->model('formation_model_search');
...
;        }
			...
			$data['total_formation'] = $this->formation_model_search->record_count($key);
			$config = array();
			$config["base_url"] = base_url() . "index.php/formation/index";
			$total_row = $this->formation_model_search->record_count($key);
			...
			$arr["something"] =  $this->formation_model_search->get_formation(FALSE, $config["per_page"], $page, $key);
			...
			$data['regex_alt'] = $this->formation_model_search->get_regex_alt();

      $this->load->view('_base/head',$data);
