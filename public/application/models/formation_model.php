...
class Formation_model extends CI_Model {
    ...
    public function get_formation_by_filter($id_type_formation,$id_domaine,$id_composante,$id_filiere,$id_diplome,$id_site,$id_type_stage,$id_rythme,$key) {

        $this->db->select('rd_formation.id as id, type_periode.nom as nom_periode,rd_formation.id_rythme as id_rythme, nbre_entreprise, rd_formation.id_domaine,'
            . 'nbre_ecole, libelle as libelle,niveau, composante.nom as nom_c, composante.sigle as nom_s,filiere.nom as nom_f, rd_formation.id_type_formation,'
            . ' type_stage.nom as nom_stage,rd_formation.id_diplome, rd_formation.id_site, regex_stage, regex_alternance,'
            . 'nbre_ecole,nbre_semaine, debut_stage, fin_stage, rd_formation.est_alternance,rd_formation.id_filiere,'
            . 'diplome.nom as nom_d, domaine.nom as nom_do,rd_formation.id_composante, skills, '
            . 'site.nom as nom_site, rd_formation.detail_stage, rd_formation.detail_alt, rd_formation.nbre_modif');
        $this->db->from('rd_formation');
        $this->db->join('composante', 'composante.id = rd_formation.id_composante');
        $this->db->join('filiere', 'filiere.id = rd_formation.id_filiere');
        ...
        if($id_type_stage != 0){
            $this->db->where('rd_formation.id_type_stage', $id_type_stage);}
        if($id_rythme != 0){
            $this->db->where('rd_formation.regex_alternance', $id_rythme);}
        $query = $this->db->get();
        return $query->result();
    ...
    public function get_formation_by_id($id){
            ...
            $this->db->select('rd_formation.id as id, type_periode.nom as nom_periode,rd_formation.id_rythme as id_rythme, nbre_entreprise, rd_formation.id_domaine,'
                . 'nbre_ecole, skills, libelle as libelle,niveau, composante.nom as nom_c, composante.sigle as nom_s,filiere.nom as nom_f, rd_formation.id_type_formation,'
                . ' type_stage.nom as nom_stage,rd_formation.id_diplome, rd_formation.id_site, regex_stage, regex_alternance,'
                . 'nbre_ecole,nbre_semaine, debut_stage, fin_stage, rd_formation.est_alternance,rd_formation.id_filiere,'
    ...
    public function _get_formation_by_id($id = 0) {
        if ($id === 0) {
            $this->db->select('*');
            $this->db->from('formation');
            $this->db->join('composante', 'composante.id = formation.id_composante');
